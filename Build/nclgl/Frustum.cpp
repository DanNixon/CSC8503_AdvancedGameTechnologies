...
void Frustum::FromMatrix(const Matrix4 &mat)
{
  ...
  auto mtx_idx = [&](int col, int row) { return col * 4 + row; };

  auto build_plane = [&](int row, int sign) {
    return Plane(Vector3(mat[mtx_idx(0, 3)] + sign * mat[mtx_idx(0, row)], mat[mtx_idx(1, 3)] + sign * mat[mtx_idx(1, row)],
                         mat[mtx_idx(2, 3)] + sign * mat[mtx_idx(2, row)]),
                 mat[mtx_idx(3, 3)] + sign * mat[mtx_idx(3, row)], true);
