...
public:
  ...
  {
    return *((Vector3 *)&mat_array[idx * 3]);
  }
  inline void SetCol(int idx, const Vector3 &row)
  {
    memcpy(&mat_array[idx * 3], &row.x, 3 * sizeof(float));
  }

  inline Vector3 GetRow(int idx) const
  {
    return Vector3(mat_array[idx], mat_array[3 + idx], mat_array[6 + idx]);
  }
  inline void SetRow(int idx, const Vector3 &col)
  {
    mat_array[idx] = col.x;
