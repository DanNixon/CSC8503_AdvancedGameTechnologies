...
void OGLRenderer::InitializeOGLContext(Window &window)
{
  ...
  pfd.dwFlags = PFD_DOUBLEBUFFER | PFD_SUPPORT_OPENGL |
                PFD_DRAW_TO_WINDOW; // It must be double buffered, it must support OGL(!), and it must allow us to draw to it...
  ...
  // Everywhere else in the Renderers, we use function pointers provided by GLEW...but we can't initialise GLEW yet! So we have to
  // use the 'Wiggle' API
  ...
  glewExperimental =
      GL_TRUE; // This forces GLEW to give us function pointers for everything (gets around GLEW using 'old fashioned' methods
...
#ifdef OPENGL_DEBUGGING
#if _DEBUG
  // PFNWGLCREATECONTEXTATTRIBSARBPROC glDebugMessageCallbackTEMP = (PFNWGLCREATECONTEXTATTRIBSARBPROC)
...
void OGLRenderer::DrawDebugBox(DebugDrawMode mode, const Vector3 &at, const Vector3 &scale, const Vector3 &colour)
{
  ...
  target->AddLine(at + Vector3(-scale.x * 0.5f, scale.y * 0.5f, 0), at + Vector3(-scale.x * 0.5f, -scale.y * 0.5f, 0), colour,
                  colour);
...
void OGLRenderer::DrawDebugCircle(DebugDrawMode mode, const Vector3 &at, const float radius, const Vector3 &colour)
{
  ...
  for (int i = 0; i < stepCount; ++i)
  {
    ...
    target->AddLine(Vector3(startx, starty, at.z), Vector3(endx, endy, at.z), colour, colour);
  }
}
