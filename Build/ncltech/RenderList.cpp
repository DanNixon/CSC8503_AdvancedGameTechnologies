...
void RenderList::UpdateCameraWorldPos(const Vector3 &cameraPos)
{
  ...
  // Note: For transparent objects we want to sort them back to front, so in
  // order to keep it simple and not change the sorting proceedure we just
  // invert them distances.
  //		 This means that negative far distance is now less than negative near
  // distance.
...
void RenderList::RemoveExcessObjects(const Frustum &frustum)
{
  auto mark_objects_for_removal = [&](std::vector<RenderList_Object> &list) {
    // First iterate over each object in the list and mark it for removal (this
    // can easily be parallised as it does not need any synchronisation)
...
#pragma omp parallel for
    ...
    // Next iterate over the list - removing any objects that are no longer
    // inside the frustum
    int n_removed = 0;
    for (int i = 0; i < size; ++i)
    {
...
void RenderList::InsertObject(Object *obj)
{
  ...
  {
    // No more list changes this frame - all new elements will have to be added
    // over future frames
    return;
  }
  ...
  if (!isOpaque)
  {
    ...
    if (!isOpaque)
    {
      ...
      // To cheat the sorting system to always use the same sorting opperand, we
      // just invert all transparent distances so negative far is less than neg
      // near.
...
void RenderList::RemoveObject(Object *obj)
{
  ...
  if (found)
  {
    ...
    // Move all elements after the 'to be deleted element' forward, so we can
    // safely pop the last element without losing anyone
