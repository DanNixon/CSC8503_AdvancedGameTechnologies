...
public:
  ...
  //	- This can be called multiple times per screen render, or none at all if
  // the object is not in any visible view frustum
  //	  so please don't put any game logic in here.
  virtual void OnRenderObject(){};

  // Called when the object should be updated
  //	- This is called once per frame regardless of screen-visibility and should
  // handle all object logic/AI
  virtual void OnUpdateObject(float dt){};

  //<----- MOUSE INTERACTIVITY --------->
  //	- To Enable mouse interactivity the object must call
  //"ScreenPicker::Instance()->RegisterObject(this)"
  //    in the constructor (or as required). It renders the object to the screen
  //    via OnRenderObject() so
  //    will only recieve mouse interactivity callbacks if it renders something
  //    to the screen.
