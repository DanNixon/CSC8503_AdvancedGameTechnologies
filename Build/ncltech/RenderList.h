...
// - Will be added on future frames instead to share workload and prevent
// lock-ups
#define MAX_LIST_CHANGE_PER_FRAME 300

// Sort opaque objects front to back to reduce over drawing - tie up between
// slow sorting or slow rendering, in the current
// usage the sorting is almost always the bottlekneck and overdraw on the Gfx
// card is negligible.
// - Transparent objects however always need to be sorted in order to correctly
// blend with background objects.
...
public:
  ...
  // Sort lists based on camera position using insertion sort. With frame
  // coherency,
  // the list should be 'almost' sorted each frame and only a few elements need
  // to be swapped in this function.
  void SortLists();
  ...
  // Misc. Removes a single object from the list, in general just call
  // 'RemoveExcessObjects' and let it remove the object automatically
...
protected:
  ...
  // maintain the hard limit on 32 renderlists.
  static uint g_NumRenderLists;

  uint m_NumElementsChanged;

  // Bit mask for renderlist, uint leads to 32 booleans (1's or 0's) thus there
  // is a hardlimit of 31 shadow maps along with the main camera render list.
  uint m_BitMask;

  // If false - all transparent objects will be ignored (maybe shadow render
  // passes?)
...
private:
  // Private Constructor - Allocate through 'AllocateNewRenderList' factory
  // method
  RenderList();
