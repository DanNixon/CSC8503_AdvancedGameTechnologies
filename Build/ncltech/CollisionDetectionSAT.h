...
public:
  ...
  void BeginNewPair(PhysicsObject *obj1, PhysicsObject *obj2,
...
protected:
  ...
  // - This only inserts the axis if it is not a duplicate of pre-existing
  // collision axis
  bool AddPossibleCollisionAxis(Vector3 axis);

  // This will build a list of all possibly colliding axes between the two
  // objects
  ...
  // Performs a plane/edge collision test, if an intersection does occur then
  //    it will return the point on the line where it intersected the given
  //    plane.
  Vector3 PlaneEdgeIntersection(const Plane &plane, const Vector3 &start,
                                const Vector3 &end) const;

  // Performs sutherland hodgeson clipping algorithm to clip the provided mesh
  //    or polygon in regards to each of the provided clipping planes.
  void SutherlandHodgmanClipping(const std::list<Vector3> &input_polygon,
                                 int num_clip_planes, const Plane *clip_planes,
