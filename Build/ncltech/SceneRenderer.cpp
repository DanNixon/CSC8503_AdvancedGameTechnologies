...
void SceneRenderer::FreeObjectFromRenderLists(Object *obj)
{
  ...
  for (size_t i = 0; i < m_ShadowMapNum; i++)
    m_apShadowRenderLists[i]->RemoveObject(obj);
}
    ...
    , m_GammaCorrection(1.0f / 2.2f)
{
  ...
  // We do not have an OGLContext at this point so nothing can be loaded
  ...
  for (uint i = 0; i < m_ShadowMapNum; ++i)
  {
    ...
    {
      NCLERROR("Unable to allocate shadow render list %d! - Try using less "
               "shadow maps",
               i);
    }
  }
...
void SceneRenderer::RenderScene()
{
  ...
  glBindTexture(GL_TEXTURE_2D, m_ScreenTex[SCREENTEX_COLOUR1]);
...
void SceneRenderer::DeferredRenderOpaqueObjects()
{
  ...
  glBindTexture(GL_TEXTURE_2D, m_ScreenTex[SCREENTEX_COLOUR0]);
  glActiveTexture(GL_TEXTURE7);
  glBindTexture(GL_TEXTURE_2D, m_ScreenTex[SCREENTEX_NORMALS]);
  ...
  glBindTexture(GL_TEXTURE_2D, m_ScreenTex[SCREENTEX_COLOUR0]);
  glActiveTexture(GL_TEXTURE6);
  glBindTexture(GL_TEXTURE_2D, m_ScreenTex[SCREENTEX_LIGHTING]);
  glDrawArrays(GL_POINTS, 0, 1);
