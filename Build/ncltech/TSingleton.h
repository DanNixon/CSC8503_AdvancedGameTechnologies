...
public:
  ...
  static T *Instance()
  {
    // This if statement prevents the costly thread Lock-step being required
    // each time the instance is requested
    if (!m_pInstance)
    {
      // Lock /is/ required here though, to prevent multiple threads
      // initialising multiple instances
      // of the class when it turns out it has not been initialised yet
      std::lock_guard<std::mutex> lock(m_Constructed);

      // Check to see if a previous thread has already initialised an instance
      // in the time it took
      // to acquire a lock.
      if (!m_pInstance)
      {
  ...
  static void Release()
  {
    // Technically this could have another enclosing if statement, but speed is
    // much less of a problem as
...
// Finally make sure that the instance is initialised to NULL at the start of
// the program
