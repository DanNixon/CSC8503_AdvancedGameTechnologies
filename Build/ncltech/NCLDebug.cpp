                          ...
                          const Vector4 colour)
{
  ...
  for (int i = 0; i < text_len; ++i)
  {
    ...
    m_vChars.push_back(colour); // We dont really need this, but we need the
                                // padding to match the same vertex format as
                                // all the other debug drawables
...
void NCLDebug::DrawDebugLists()
{
  ...
  auto render_drawlist = [&](NCLDebug::DebugDrawList &list, size_t *offsets) {
        ...
        (list._vHairLines.size() + list._vTris.size()) > 0)
    {
      ...
      if (!list._vTris.empty())
        glDrawArrays(GL_TRIANGLES, offsets[3] >> 1, list._vTris.size() >> 1);
    }
  };
...
// This Function has been abstracted from previous set of draw calls as to avoid
// supersampling the font bitmap.. which is bad enough as it is.
