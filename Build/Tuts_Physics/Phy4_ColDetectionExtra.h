...
public:
      ...
      , m_MeshGarden(NULL)
  {
    ...
    dTex = SOIL_load_OGL_texture(
        TEXTUREDIR "raptor.jpg", SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID,
  ...
  virtual void OnInitializeScene() override
  {
    PhysicsEngine::Instance()->SetDebugDrawFlags(
        DEBUGDRAW_FLAGS_COLLISIONNORMALS | DEBUGDRAW_FLAGS_COLLISIONVOLUMES);
    ...
    this->AddGameObject(CommonUtils::BuildCuboidObject(
        "Ground", Vector3(0.0f, -1.001f, 0.0f), Vector3(20.0f, 1.0f, 20.0f),
    ...
    player->Physics()->SetCollisionShape(
        new CuboidCollisionShape(Vector3(0.5f, 0.5f, 1.0f)));
    ...
    // Create Some Objects
    {
      ...
      obj->SetLocalTransform(
          Matrix4::Translation(Vector3(0.0f, -0.71f, 0.0f)) *
          Matrix4::Scale(Vector3(2.0f, 2.0f, 2.f))); // Translation here to move
                                                     // the mesh down, as it not
      ...
      obj->Physics()->SetOnCollisionCallback(
          [](PhysicsObject *self, PhysicsObject *collidingObject) {
            NCLDebug::Log("You are inside the house!");

            // Returns false if the collision should /not/ be handled with a
            // standard collision response (tut 6)
...

    {
      ...
      obj->SetLocalTransform(
          Matrix4::Translation(Vector3(0.0f, -0.5f, 0.0f)) *
          Matrix4::Scale(Vector3(2.0f, 1.0f, 2.f))); // Translation here to move
                                                     // the mesh down, as it
                                                     // /also/ not centred on
                                                     // the origin *cry*
    ...
    //'Hidden' Physics Node (no visual mesh or renderable object attached)
    {
      ...
      obj->SetOnCollisionCallback(
          [obj](PhysicsObject *self, PhysicsObject *collidingObject) {
